...
/**
 * Class to check the semantics of the given exists expression.
 *
 * This will only allow tag as it's operand.
 *
 * @author Atul M Dambalkar (atul@entrib.com)
 */
public class ExistsExpressionSemanticsChecker {

    /**
     * Only allow Tag value
     * @param expression
     * @return
     */
    public static boolean checkSemantics(Expression expression) {
        return ((UnaryExpression)expression).getChild().getExpressionTypeEnum()
                == Expression.ExpressionTypeEnum.IdentifierValue;
    }
